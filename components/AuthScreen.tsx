...
function SignInCard({ initialFlow }: { initialFlow: Flow }) {
  ...
  function handleGoogle() {
    setError(null);
    setSubmitting(true);
    // Redirects away from the page, so there is no success path to handle.
    void signIn("google", { redirectTo: "/documents" }).catch(() => {
      ...
      {step.kind === "credentials" && (
        <form onSubmit={handleCredentials} className={formClass}>
          <button
            ...
            disabled={submitting}
            className="w-full flex items-center justify-center gap-2 rounded-lg border border-slate-300 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-50 transition-colors"
          >
